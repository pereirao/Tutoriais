# minibar-store-partners
React-based Supplier Dashboard

## Deploying
 The app injects environment variables at the build process which is a compile, not run time, step.  So When deploying to various platforms on Heroku etc. be careful not to merely promote a build from one machine to another if you actually intend for them to incorporate different environment variables.
 
 Best to push to Staging from the console using the Heroku Platform API, and when ready to promote to Prod, push to Prod the same way. When pushing updates to the production application, be sure to rev the version number in the package.json per our usual treatment of Major.Minor.Build versioning. This package.json version will be added as an "app-version" header on all Supplier API v2 requests, and will also be the comparison string used when receiving a `prompt_update` socket message from the Minibar Notification Server. 

mb-dev-supplier should point to sandbox.minibardelivery.com/api/partners/v2
mb-supplier should point to api.minibardelivery.com/api/partners/v2
## Create React App Info VVV
